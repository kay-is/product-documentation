...
To ensure blocking of specific sources, **you must also check for the `penalty` tag**, either by defining a separate rule that blocks requests with the `penalty` tag or by adding the tag check to the existing rule. 

#### Block all pentalty requests

The following rule will block all requests that have the `penalty` tag. This ensures `block` actions triggered by any rule will be enforced.

Creating a separate rule for the `penalty` tag is useful when you exhauseted the 5 tags limit in a single rule.
...
In this example, the check for the `penalty` tag is embedded in a specific rule, so it won't be necessary to create a separate rule to block requests with the `penalty` tag.
